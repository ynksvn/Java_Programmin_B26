...
/*Collection: the parent of all other collections types. The interface defines the most common action/methods that a collection should have

list: ordered/index, allow duplicates

set: unordered/no index, does not allow duplicates

ArrayList: data structure that uses arrays internallu, Resizeable

LinkedList: data structure that uses nodes internally

Vector: legacy version of ArrayList. It uses array internallu, but its synchronized/thread safe
Thread safe: Thread safety implies multiple threads can write/read data in same object without memory inconsistency errors. In highly multi threaded program
